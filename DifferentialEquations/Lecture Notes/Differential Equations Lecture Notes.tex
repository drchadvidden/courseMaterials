...
\item Example: Population growth
\[
\frac{dP}{dt} = 2P
\]
$P(t)$ is an unknown population function of time $t$. Many solutions, but can write the general solution. Given an initial population $P(0) = P_0$, we get a unique solution. Integration techniques give our solution in this case (analytic techniques).

\item General first order (derivative) equation: Notice we focus on change, not static values like an algebraic equation.
\[
\frac{dy}{dt} = f(t,y)
\]
$f(t,y)$ is some given function. Generally no technique can solve, so we will need other ideas (numerical and qualitative analysis).
...
\item Goal is to model the real world which involves making assumptions, building equations, and analysis to hopefully find solutions. 

\item Useful in all sciences (population growth, disease spread, fluid dynamics, heat transfer, ecology, circuits, chemical reaction, stock market, etc)

\item List is long: \url{https://en.wikipedia.org/wiki/List_of_nonlinear_partial_differential_equations}

\item Trouble is almost none can be solved, instead they are analyzed in general (theory) and approximated via simulations (coding).
\end{enumerate}

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\item Course topics:
\begin{enumerate}
\item Building (analyzing) mathematical models (general theme)
\item Ch1: First-order differential equations
\item Ch2: First order systems
\item Ch3: Linear systems
\item Ch4: Forcing and resonance
\item Ch5: Nonlinear systems
\item Ch6: Laplace transform
\end{enumerate}
...
\begin{enumerate}
\item A \textbf{mathematical model} translates a real-world problem into mathematics.
\item Assumptions simplify reality and determine the model.
\item More assumptions $\Rightarrow$ simpler mathematics, but often a less realistic model.
\item The goal is to \textbf{understand, explain, and predict} behavior—not merely solve an equation.
\end{enumerate}
...
Assumption: The growth rate is proportional to the population.
...
\begin{itemize}
\item First-order linear ODE.
\item Unknown: the function $P(t)$.
\item General solution:
\[
P(t)=Ce^{kt}.
\]
\item An initial condition, e.g. $P(0)=P_0$, gives a unique (particular) solution.
\item The parameter $k$ represents the growth rate. This needs to be determined via empirical rule or data.
\item We will study:
    \begin{itemize}
    \item analytical solutions (formula for $P$),
    \item qualitative behavior (effects of different $P_0$, draw various cases, note equilibrium case),
    \item numerical approximations (if analytical solutions cannot be found, we will use Python for this).
    \end{itemize}
\end{itemize}

\item Logistic Growth (refinement of above)

Assumption: Growth slows as the population approaches a carrying capacity $N$.
...
\begin{itemize}
\item First-order nonlinear ODE.
\item Equilibrium solutions occur where $f(P)=0$.
\item Graph $f(P)$ to determine increasing/decreasing solutions.
\item More realistic than exponential growth, though more complicated.
\end{itemize}

\item General First-Order ODE
...
\begin{itemize}
\item Describes how $y$ changes.
\item Much of this course studies equations of this form.
\item Initial value problem case (IVP).
\item Independent variable: usually time $t$.
\item Dependent variable: the unknown function, e.g. $y(t)$.
\item Variables vs. parameters
\begin{itemize}
\item Variables change during the problem ($t$, $y$).
\item Parameters are fixed constants that describe the system ($k$, $N$, ...).
\item Changing a parameter gives a different model.
\end{itemize}
\end{itemize}
...
\begin{itemize}
\item Many systems involve several interacting variables.
\item Let $R(t)$ = rabbits and $F(t)$ = foxes.
\end{itemize}

Assumptions:
\begin{itemize}
\item Rabbits reproduce naturally.
\item Rabbits decrease through predation.
\item Foxes die naturally.
\item Foxes increase by consuming rabbits.
\end{itemize}

\[
\begin{cases}
\dfrac{dR}{dt}=\alpha R-\beta RF,\\[1ex]
\dfrac{dF}{dt}=-\gamma F+\delta RF.
\end{cases}
\]

\begin{itemize}
\item First-order, coupled nonlinear system.
\item This gives a coupled nonlinear system and equilibrium solutions have new interest. Play around with inc/dec rabbit/fox populations.
\item Variables cannot be solved independently.
\item We will study equilibria, stability, and long-term behavior.
\item Linear systems can often be analyzed using linear algebra (linear approximations).
\end{itemize}
...
\begin{enumerate}
\item There are few nice cases of first order DEs which we can completely solve by hand. These are important to gain insight from.
\item Separable equation: $f$ can be separated as a product of a function of $t$ and function of $y$.
\[
\frac{dy}{dt} = f(t,y)= g(t)h(y)
\]
\item Once separated the solution can be found via direct integration (if doable). Our focus is then just on calculus.
\end{enumerate}
...
y = y_0e^{t^2}.
\]
Note, solutions are easy to verify via the original equation.

%%%%%%%%%%%%%%%%%%%%%%%%%%
\item Exponential growth: An autonomous equation of the form $\frac{dy}{dt} = f(y)$. The right hand side depends on $y$ alone.
\[
\frac{dy}{dt} = ky
\]

%%%%%%%%%%%%%%%%%%%%%%%%%%
\item Logistic growth: Another autonomous equation.
\[
...
\]
Again we are separable, but note the right hand side only depends on $P$. This is called an \emph{autonomous} equation. We solve the same way. Note the equilibrium solutions $P=0,1$ as we divide.
\begin{align*}
...
\ln|y| + \frac{y^2}{2} = t+C
\]
and it is not possible to solve for $y$ (known as an implicit solution). Like the equation of a circle, this still completely describes the solution set. We cannot write $y$ explicitly as a function of $t$. Also, $y=0$ is an \emph{equilibrium solution} which we missed due to (zero) division, yet our implicit solution does not make sense for $y=0$. Further, there are examples where integration is not possible. Don't forget to check for equilibrium solutions!
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\item Analytic solutions are often impossible (or difficult) to obtain. Instead, we can study a differential equation qualitatively.
\begin{itemize}
\item Visualize solutions using a \emph{slope field} (or \emph{direction field}).
\item Approximate solutions numerically.
\item Predict long-term behavior (equilibria and stability).
\end{itemize}

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\item Slope fields visualize the differential equation
\[
\frac{dy}{dt}=f(t,y).
\]
At each point $(t,y)$, draw a short line segment with slope
\[
m=f(t,y).
\]
Solution curves are tangent to these line segments everywhere.

%%%%%%%%%%%%%%%
\item Example: $\displaystyle y'=2y,\qquad y(0)=1$
\begin{itemize}
\item Slopes depend only on $y$ (autonomous equation), so each horizontal row has identical slope.
\item Slopes are positive above $y=0$, negative below, and zero on $y=0$.
\item Sketch the solution through $(0,1)$.
...
\begin{itemize}
\item Nullcline: $y=2$.
\item Above $y=2$, slopes are negative.
\item Below $y=2$, slopes are positive.
\item Every nearby solution moves toward $y=2$, so it is a stable equilibrium.
\item Exact solution:
\[
y=2+Ce^{-t}.
\]
...
\item Isoclines
\begin{itemize}
\item An isocline is a curve where the slope is constant:
\[
f(t,y)=s.
\]
\item The nullcline is the special isocline where
\[
f(t,y)=0.
\]
\item Plotting several isoclines makes hand sketches much easier.
\end{itemize}
...
\begin{itemize}
\item Nullclines: $P=0$ and $P=1$.
\item Slopes are positive for $0<P<1$.
\item Slopes are negative elsewhere.
\item Therefore
\begin{itemize}
\item $P=1$ is stable.
\item $P=0$ is unstable.
\end{itemize}
\item Compare with the familiar logistic curve.
\end{itemize}
...
y=t+1+s.
\]
\item Unlike autonomous equations, the nullcline is not horizontal.
\item Solutions approach the line
\[
y=t,
\]
which is an exact (nonconstant) solution.
\end{itemize}
...
\]
\begin{itemize}
\item Slopes depend only on $t$.
\item Every vertical column has the same slope.
\item Isoclines are vertical lines.
\item Example:
\[
y'=2t,
\]
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\item Using \texttt{dfield}

\begin{itemize}
\item Computes slope fields by evaluating $f(t,y)$ on a grid.
\item Numerically approximates solution curves from chosen initial conditions.
\item Useful for exploring equations before attempting analytic methods.
\end{itemize}
...
\begin{itemize}
\item A slope field visualizes the differential equation itself.
\item Solution curves are everywhere tangent to the direction arrows.
\item Isoclines organize regions of equal slope; nullclines identify zero slope.
\item Autonomous equations have horizontal isoclines and equilibrium solutions.
\item Qualitative analysis reveals long-term behavior even when no explicit solution exists.
\end{itemize}

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\item A differential equation tells us how a solution changes, not what the solution is. A slope field converts this local information into a global picture of all possible solutions.
\end{enumerate}
...
\begin{itemize}
\item We trade exact for approximate, though as close to the exact solution as desired if we are persistent.
\item The simplest idea harnesses tangent lines (just like direction fields) and is called Euler's method.
\item Drawback with this simple idea will be inefficiency, but this can be improved with more advanced methods which can be derived and analyzed via calculus ideas (Taylor series, numerical analysis).
\end{itemize}
...
\begin{enumerate}
%%%%%%%%%%%%%%%%%%%
\item Idea: Approximate the derivative (tangent line slope) with a difference quotient (secant line slope):
\[
\frac{df}{dx} = \lim_{h \rightarrow 0} \frac{\Delta f}{\Delta x}  
= \lim_{h \rightarrow 0} \frac{f(x+h)-f(x)}{h}  
= \lim_{\Delta x \rightarrow 0} \frac{f(x+\Delta x)-f(x)}{\Delta x} 
\]
translated to
\[
\frac{dy}{dt} =  \lim_{\Delta t \rightarrow 0} \frac{y(t+\Delta t)-f(t)}{\Delta t} \approx  \frac{y(t+\Delta t)-y(t)}{\Delta t}  
\]
for some $t$ and $\Delta t$. Fitting into our equation,
\[
\frac{y(t+\Delta t) - y(t)}{\Delta t} \approx f(t,y)
\]
gives a way to approximate the future
\[
...
\begin{itemize}
\item Notation: $y(0)=y_0$ (given initial value), $y_1 \approx y(t_1)$, $t_1 = t_0+\Delta t$ (some chosen time step), $f_0=f(t_0, y_0)$ (slope of the solution curve at $t_0$). Likewise for $t_n, y_n$ and $f_n$.
\item Follow the tangent line to get the first approximation:
\[
y_1 = y_0 + \Delta t f_0
\]
\item Example: $y'=2y$ with $t_0=0, y(t_0)=1$. Then the slope of the tangent is $f_0=2$ and the tangent line is $y=y_0+f_0(t-t_0)$ giving 
\[
...
&= (1+2\Delta t)^n y_0 \\
\end{align*}
If $y(0)=1$, say we are truly after an approximation to $y(1)$. Then if we take $n+1$ time steps ($\Delta t = \frac{1}{n}$)
\[
...
\item Draw on $y'=2y$ example 2 steps to $T=1$ via 4 steps. Close approximation in the end, but how much better?
\end{itemize}

\end{enumerate}

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\item General Euler's method algorithm: Given the initial value problem
\[
\frac{dy}{dt}=f(t,y), \qquad y(t_0)=y_0,
\]
and a chosen step size $\Delta t$:
\begin{enumerate}
\item Set
\[
t=t_0,\qquad y=y_0.
\]
\item Repeat until the desired final time is reached:
\[
\boxed{
\begin{aligned}
t_{n+1} &= t_n+\Delta t,\\
...
\item The computed values
\[
(t_0,y_0),\ (t_1,y_1),\ \ldots,\ (t_n,y_n)
\]
approximate the true solution at the times
\[
t_n=t_0+n\Delta t.
\]
\end{enumerate}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\item Example: Random homework problem.

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\item Idea of numerical analysis:
\begin{enumerate}
\item Build mathematical guarantees about a numerical method such as Euler's method (simplest explicit method).
\item Local error (per iteration), global error (accumulation of local errors)
\item Convergence, rate of convergence (under suitable conditions, as $\delta t \rightarrow 0$ we converge to the true solution, rate of Euler's method is first order accurate, halving $\Delta t$ roughly halves the error)
\item Stability analysis (conditions for convergence)
\item Higher order methods (more terms, better approximation, faster convergence)
\item Key tool: Taylor series and Taylor's Theorem (Chapter 7 holds mathematical analysis)
\end{enumerate}

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\item Programming assignment: Part 1, reproduce results in PDF. Details on Canvas.
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\item \textbf{The basic question:} Given an initial value problem, can we be sure that a solution exists? If it exists, is it the \emph{only} solution?
    
These are theoretical questions, but they have practical consequences. If a differential equation is being used as a model, we want to know that the model actually determines a solution. This is also important when using numerical methods: if the IVP does not have a well-defined solution, there is nothing for a numerical method to approximate.

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\item \textbf{We have seen questions like this before.}
\begin{enumerate}
    \item Quadratic equations: How many solutions does
    \[
    ax^2+bx+c=0
    \]
    have? How do we know?
    
    \item Linear systems: When does
    \[
    A\vec{x}=\vec{b}
    \]
    have a solution? When is the solution unique?
    \[
    \det(A)\neq 0,\qquad \text{full rank},\qquad
    \text{linearly independent columns},\ldots
    \]
    
    \item Differential equations: Given
    \[
    \frac{dy}{dt}=f(t,y),\qquad y(t_0)=y_0,
    \]
    when can we be sure that a solution exists? When is it unique?
\end{enumerate}

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\item \textbf{What can go wrong?}

Consider an initial value problem. We might encounter:
\begin{itemize}
    \item no solution;
    \item more than one solution;
    \item a solution that exists only for a limited range of $t$.
\end{itemize}

The goal of this section is to identify conditions that rule out these problems.

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\item \textbf{Why does uniqueness matter?}
...
Suppose a solution starts at $y(0)=\frac12$. Since
\[
y(1-y)>0\qquad\text{for }0<y<1,
\]
the solution initially increases.

\begin{quote}
Could this solution ever cross $y=1$?
\end{quote}

If it did, at the moment it reached $y=1$ there would be two different solutions through the same point: our solution and the equilibrium solution $y=1$.

Thus, if solutions are unique, \textbf{solution curves cannot cross}. In particular,
\[
0<y(t)<1
\]
for all $t$ for which the solution exists.

This illustrates the practical importance of uniqueness: it lets us draw conclusions about the behavior of solutions without solving the differential equation.

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\item \textbf{Existence:} Consider the IVP
\[
\frac{dy}{dt}=f(t,y),\qquad y(t_0)=y_0.
\]
The basic idea is that $f$ needs to be ``nice enough'' near the initial point.
\begin{enumerate}
    \item \textbf{Existence Theorem:} Suppose $f(t,y)$ is continuous on a rectangle
    \[
    R=\{(t,y):a<t<b,\ c<y<d\}.
    \]
    If $(t_0,y_0)$ is in $R$, then there exists a solution $y(t)$ to the IVP for $t$ sufficiently close to $t_0$.

    \item ``Sufficiently close'' means that there is some $\epsilon>0$ such that the solution exists for
    \[
    |t-t_0|<\epsilon.
    \]
    The theorem guarantees existence \emph{locally}; it does not necessarily guarantee that the solution exists for all time.

    \item If $f$ is not continuous near $(t_0,y_0)$, this theorem does not apply. A solution may or may not exist.
    ...
    y(t)=\tan(t).
    \]
    Thus a solution exists near $t=0$, as the theorem predicts, but it blows up at
    \[
    t=\frac{\pi}{2}.
    \]
    Increasing $y$ generates a quadratic increase in $\frac{dy}{dt}$ which gets out of hand.
    So existence near the initial point does not imply existence for all $t$.
\end{enumerate}

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\item \textbf{Uniqueness:} Existence is only part of the story. Suppose we know that a solution exists. Could there be several different solutions passing through the same initial point?
\begin{enumerate}
    \item \textbf{Existence and Uniqueness Theorem:} Suppose $f(t,y)$ and
    \[
    \frac{\partial f}{\partial y}(t,y)
    \]
    are continuous on a rectangle $R$ containing $(t_0,y_0)$. Then the IVP has a unique solution for $t$ sufficiently close to $t_0$.

    \item The condition on $\partial f/\partial y$ gives us a way to guarantee that two different solution curves cannot pass through the same initial point.

    \item \textbf{Existence does not guarantee uniqueness.} Consider
\[
\frac{dy}{dt}=3y^{2/3},\qquad y(0)=0.
\]
Here $f(y)=3y^{2/3}$ is continuous, so the existence theorem guarantees a solution. However, the solution is not unique. For example,
\[
y(t)=0
\qquad\text{and}\qquad
y(t)=t^3
\]
both satisfy the IVP. In fact, a solution can remain at $0$ for any amount of time and then depart:
\[
y(t)=
\begin{cases}
0, & t\leq C,\\
...
\]
is not continuous at $y=0$, so the uniqueness theorem does not apply.

\item This example shows why existence and uniqueness are separate questions:
\begin{itemize}
    \item $f$ is continuous $\Rightarrow$ existence is guaranteed;
    \item $f_y$ is not continuous at the initial point $\Rightarrow$ uniqueness is not guaranteed.
\end{itemize}

Thus, ``a solution exists'' does not necessarily mean ``the initial condition determines a single solution.''

    \item Example:
    \[
    \frac{dy}{dt}=\frac{t}{y},\qquad y(0)=0.
    \]
    Here $f(t,y)=t/y$ is not defined at the initial point. For $t\neq0$, however, both
	\[
	y(t)=t
	\qquad\text{and}\qquad
	y(t)=-t
	\]
	satisfy the differential equation, and both approach $y=0$ as $t\to0$.

	Thus the point $(0,0)$ lies on multiple solution curves of the differential equation, but strictly speaking these are not classical 	solutions of the IVP on an interval containing $t=0$, since the differential equation is undefined at the initial point.

	This illustrates why the hypotheses of the theorem matter: if $f$ is not defined at the initial point, we cannot use the theorem to 	guarantee existence or uniqueness.
\end{enumerate}

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\item \textbf{Why uniqueness matters:} Suppose $f$ is nice enough that the existence and uniqueness theorem applies. Then:
\begin{quote}
If two solution curves ever pass through the same point at the same time, they must be the same solution.
\end{quote}
This simple idea gives us a powerful way to understand solutions without solving the differential equation explicitly.

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\item \textbf{Example: the logistic equation}
...
and
\[
f(y)=y(1-y)>0\qquad\text{for }0<y<1,
\]
the solution initially increases.

Could it cross $y=1$?

No. If it reached $y=1$ at some time $t_1$, then there would be two solutions passing through the same point $(t_1,1)$:
\[
y(t)=1
\]
and our solution. Uniqueness says this is impossible unless the two solutions are identical. But our solution started at $y(0)=1/2$, so they cannot be identical.

Similarly, the solution cannot cross $y=0$.

Therefore,
\[
0<y(t)<1
\]
for all $t$ for which the solution exists. Since $y'(t)>0$ in this interval, the solution is always increasing.

This gives us qualitative information about the solution without solving the differential equation.

In fact, the solution approaches $1$ as $t\to\infty$. If it approached some value $L<1$, then
\[
f(L)=L(1-L)>0,
\]
so the solution would still have a positive slope near $L$ and could not settle down there.


%%%%%%%%%%%%%%%%
\item Depth: Avoiding future bad behavior (finite time blow up).
\begin{itemize}
...
\item A phase line compresses this information into a single line. It answers a
basic question: given the current value of $y$, will the solution increase or
decrease? Draw a vertical line and mark the equilibrium values. Arrows indicate
the direction of motion. Already saw this in the previous sections, though this simplification helps us gain intuition more easily.

\item \textbf{Reading a phase line:} The arrows determine the long-term
behavior of solutions without solving the differential equation. In particular,
they tell us which equilibria are attracting and which are repelling.

\item An equilibrium $y_0$ is \textbf{stable} if solutions that start
sufficiently close to $y_0$ remain close to it. It is \textbf{asymptotically
stable} if they also approach $y_0$ as $t\rightarrow\infty$.
\end{itemize} 
...
\begin{itemize}
\item Find equilibrium solutions, label on number line. Because $f$ is continuous, these are the only places $f$ can change sign assuming continuous $f$.
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\item Classification of equilibrium points:
\begin{enumerate}
\item \textbf{Sink (stable):} Solutions approach $y_0$ from both sides as
$t\rightarrow\infty$. If $f'(y_0)<0$, then $y_0$ is a sink.

\item \textbf{Source (unstable):} Solutions move away from $y_0$ on both sides
as $t\rightarrow\infty$. If $f'(y_0)>0$, then $y_0$ is a source.

\item \textbf{Semistable (stable from one side):} Solutions approach $y_0$ from one side and move away
from it on the other. If $f'(y_0)=0$, the derivative test is inconclusive;
inspect the signs of $f(y)$ on either side of $y_0$.
\end{enumerate}
...
\frac{dP}{dt}
=
kP\left(1-\frac{P}{N}\right)\left(\frac{P}{M}-1\right),
\qquad N>M>0.
\]
This is a modified logistic model with a \emph{minimum sustainable population}
(or threshold) $M$.

Choose $N=10$, $M=2$, and assume $k>0$:
\[
\frac{dP}{dt}
=
kP\left(1-\frac{P}{10}\right)\left(\frac{P}{2}-1\right).
\]

\begin{enumerate}
    \item Find and classify the equilibrium solutions.
    \item Draw the phase line.
    \item What happens to a population starting at $P(0)=1$? At $P(0)=5$?
    \item Interpret the results biologically. What role does the threshold $M$
    play?
\end{enumerate}

The equilibria are
\[
P=0,\qquad P=2,\qquad P=10.
\]

For $k>0$, the phase line is
\[
0\quad \longleftarrow\quad
2\quad \longrightarrow\quad
10\quad \longleftarrow.
\]

Thus $P=0$ and $P=10$ are sinks, while $P=2$ is a source. A population
starting below the threshold $M=2$ declines toward extinction, while a
population starting above $M$ grows toward the carrying capacity $N=10$.
...
\begin{enumerate}
    ...
    \item Draw the phase line.
    \item Classify each equilibrium as a sink, source, or semistable.
    \item Describe the long-term behavior for different initial conditions.
\end{enumerate}
...
f'(0)=f'(1)=0,
\]
so the derivative test is inconclusive. We must instead examine the sign of
$f(y)$ on either side of each equilibrium.

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\item \textbf{Connection to uniqueness:} If the hypotheses of the uniqueness
theorem hold, solution trajectories cannot cross equilibrium solutions.
Therefore, equilibrium solutions divide the phase line into regions in which
solutions remain for all future time (as long as the solution exists).
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\item \textbf{What happens when a parameter changes?}

Here we study families of differential equations that depend on one or more
parameters. A parameter can change the equilibria and, more importantly, the
...
\frac{dy}{dt}=ky,
\]
where $k$ is a parameter. For $k<0$, the equilibrium $y=0$ is a sink; for
$k>0$, it is a source. At $k=0$, every constant function is an equilibrium.

\item A \emph{bifurcation} occurs when a change in a parameter causes a
qualitative change in the behavior of solutions, such as a change in the
number or stability of equilibria.
\end{itemize}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\item \textbf{Logistic growth with harvesting.}

Consider
\[
\frac{dP}{dt}=4P-P^2-h=f(P),
\]
where $h>0$ is a constant harvesting rate.

\begin{enumerate}
\item Equilibria occur where
\[
f(P)=4P-P^2-h=0.
\]
...
\item Explore the phase line for several values of $h$:
\begin{itemize}
\item $h=0$: two equilibria (no harvesting);
\item $h=3$: two equilibria (light harvesting);
\item $h=4$: one equilibrium (critical harvesting);
\item $h=5$: no equilibria (overharvesting).
\end{itemize}

\item What happens as $h$ increases? The two equilibria approach each other,
merge at $h=4$, and then disappear. Thus $h=4$ is the \emph{bifurcation value}.

\item We can find the bifurcation value without solving for the equilibria.
At the critical value, the graph of $f(P)$ is tangent to the $P$-axis, so
\[
f(P)=0,\qquad f'(P)=0.
\]
For
...
f'(P)=4-2P.
\]
Thus $f'(P)=0$ gives $P=2$, and substituting into $f(P)=0$ gives
\[
h=4.
\]

\item \textbf{Interpretation:} $h=4$ is the maximum sustainable harvesting
rate. For $h<4$, there is a positive equilibrium population. At $h=4$, the
positive equilibria merge. For $h>4$, no positive equilibrium exists, so the
population eventually declines.

\item Draw the \emph{bifurcation diagram}: plot equilibrium values $P^*$ on
the vertical axis against the parameter $h$ on the horizontal axis. Indicate
which equilibrium branches are stable and unstable.
\end{enumerate}

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\item \textbf{Another example:} Draw and interpret the bifurcation diagram for
\[
\frac{dy}{dt}=Ay-y^2=y(A-y).
\]

\begin{enumerate}
\item Find the equilibrium solutions as functions of $A$.
\item Determine their stability for different values of $A$.
\item Identify the bifurcation value.
\item Draw and interpret the bifurcation diagram.
\end{enumerate}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\item We end the chapter with another important analytic technique. There are many techniques for solving differential equations, but two of the most widely useful for first-order equations are:
\begin{enumerate}
\item \textbf{Separation of variables}: separate the variables and integrate.
\item \textbf{Linear equations}: use the structure of the equation to construct the solution.
\end{enumerate}

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\item For linear equations, we will see two approaches:
\begin{itemize}
\item \textbf{Undetermined coefficients}: make an educated guess for a particular solution.
\item \textbf{Integrating factors}: systematically compute a particular solution (next section).
\end{itemize}

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\item \textbf{First order linear equations} can be written in the form:
\[
\frac{dy}{dt} = a(t)y + b(t)
\] 
where $a(t)$ and $b(t)$ are known functions of $t$. The important feature is that the unknown function $y(t)$ appears \emph{linearly}: it only appears to the first power and is not multiplied by another function involving $y$.
For example,
\[
y'=t^2y+\sin(t)
\]
is linear (in $y$ on the RHS), while
\[
y'=y^2+t,
\qquad
y'=\frac{1}{y}+t,
...
are nonlinear.
\begin{itemize}
\item If $b(t)=0$, the equation is called \textbf{homogeneous} (or non-forced):
\[
y'=a(t)y.
\]
\item If $b(t)\neq 0$, the equation is called \textbf{nonhomogeneous} (or forced):
\[
y'=a(t)y+b(t).
\]
The terminology ``forced'' comes from applications: $b(t)$ can represent an external input or forcing acting on the system.
\end{itemize}
...
\begin{enumerate}
%%%%%%%%%%%%%%%%
\item Theorem: Linearity principle (homogeneous general solution): If $y_h(t)$ solves the homogeneous equation $\frac{dy}{dt} = a(t)y$, then any constant times $y_h$ also solves it. Not surprising because this is a separable equation and we know:
\[
\frac{dy}{dt} = a(t)y \quad \Rightarrow \quad \int \frac{1}{y} dy = \int a(t) dt \quad \Rightarrow \quad y_h(t) = Ce^{\int a(t) dt}
\]
%%%%%%%%%%%%%%%%
\item Particular solution (to the nonhomogeneous equation): $y_p(t)$ solves the nonhomogeneous equation $\frac{dy}{dt} = a(t)y+b(t)$.
%%%%%%%%%%%%%%%%
\item Theorem: Extended linearity principle (nonhomogeneous general solution): For linear equations, adding these two parts gives the complete solution
\[
y = y_h + y_p
\]
...
\begin{itemize}
\item Undetermined coefficients (guess and check)
\item Integrating factor (next section)
\end{itemize}
\item Add two solutions per previous theorem.
\end{enumerate}
...
\item $\sin(t)$ (or $\cos(t)$) needs $\alpha \sin(t)+\beta\cos(t)$ (why both?) Example from HW.
\end{itemize}
%%%%%%%%%%%%%%%%%%%%%
...
\frac{dy}{dt} + 2y = e^t
\]
We need to multiply both sides by $e^{2t}$ (called the \emph{integrating factor}) in order to see the product rule exactly.
\begin{align*}
...
\end{align*}
as we found in the last section. Trick is to force the product rule to appear.
\end{enumerate}
...
\]
%%%%%%%%%%%%%%%%%%%%%%
\item Trouble with this technique is two potentially challenging integrations:
\begin{itemize}
