...
UMAP is a non-linear dimensionality reduction method based on manifold learning. It produces a low-dimensional embedding that preserves both local neighborhood structure and some global structure of high-dimensional data. The algorithm can be described mathematically as follows:
...
UMAP does not attempt to learn an explicit equation for the manifold. Instead, it approximates the manifold using the connectivity structure of nearby data points.

Specifically, UMAP constructs a weighted $k$-nearest neighbor graph
\[
G = (V,E),
\]
where the vertices correspond to the data points and edges connect nearby points.

Edge weights represent the strength of the local relationship between points. This weighted graph provides a discrete approximation of the manifold:

\[
M \approx G.
\]

Thus the local geometry of the manifold is captured by the neighborhood structure of the graph.

\item \textbf{Compute k-nearest neighbors:}  
\begin{itemize}
    \item For each high-dimensional point $x_i \in \mathbb{R}^D$, compute distances to all other points: $\text{dist}(x_i, x_j)$.
    \item Select the $k$ nearest neighbors: $\mathcal{N}_k(x_i)$.
    \item \emph{Whiteboard hint:} draw points in 2D, circle nearest neighbors of one point, and label the set $\mathcal{N}_k(x_i)$.
\end{itemize}
...
\begin{itemize}
    ...
    \[
    \mu_{ij} = \exp\Bigg(- \frac{\max(0, \text{dist}(x_i, x_j) - \rho_i)}{\sigma_i}\Bigg)
    \]
    where $\rho_i$ is the distance to the nearest neighbor, ensuring each point is connected, and $\sigma_i$ is set so that the effective number of neighbors is $k$:
    \[
    ...
    \begin{itemize}
        ...
        \item Fuzzy weights reflect the \emph{strength of connectivity} between points: $\mu_{ij} \in [0,1]$.
        \item Smooths noisy data and allows continuous optimization via cross-entropy.
        \item \emph{Whiteboard hint:} draw arrows of different thickness from a point to its neighbors to indicate fuzzy weights.
    \end{itemize}
\end{itemize}
...
\]
- Ensures $w_{ij} = w_{ji}$.  
- Represents a weighted, undirected graph of local connectivity.  
- \emph{Whiteboard hint:} show two asymmetric arrows combining into one symmetric edge.

\item \textbf{Define low-dimensional similarities:}  
- Let $y_i \in \mathbb{R}^d$ be low-dimensional coordinates.  
\[
v_{ij} = \frac{1}{1 + a \|y_i - y_j\|^{2b}}
\]
- $a, b > 0$ control the shape of the decay.  
- $v_{ij}$ acts like a probability of closeness in low-D.  
- \emph{Whiteboard hint:} draw two points with distance $d$ and a curve showing $v(d)$ decreasing as distance increases.

\item \textbf{Cross-entropy loss function:}  
\[
C = \sum_{i<j} \Big[ w_{ij} \log \frac{w_{ij}}{v_{ij}} + (1 - w_{ij}) \log \frac{1 - w_{ij}}{1 - v_{ij}} \Big]
\]
- First term: preserve high-weight edges (neighbors stay close).  
- Second term: push apart non-neighbors.  
- Gradient w.r.t $y_i$:
\[
\frac{\partial C}{\partial y_i} = 2 \sum_j (v_{ij} - w_{ij}) \frac{\partial v_{ij}}{\partial y_i}
\]
- \emph{Whiteboard hint:} draw points with arrows toward/away from neighbors showing forces moving them during optimization.

\item \textbf{Optimization:}  
- Use stochastic gradient descent or negative sampling to update points:
\[
y_i \gets y_i - \eta \frac{\partial C}{\partial y_i}
\]
- Local clusters form first, then global structure emerges.  
- \emph{Whiteboard hint:} show a few points moving iteratively until clusters stabilize.

\item \textbf{Key hyperparameters:}
\begin{itemize}
    \item \texttt{n\_neighbors} — controls local vs global structure.
    \item \texttt{min\_dist} — minimum spacing between points in low-D; controls cluster tightness.
    \item \texttt{metric} — high-D distance metric (Euclidean, cosine, etc.).
    \item \emph{Whiteboard hint:} draw same points with tight vs spread clusters, and show effect of changing neighbors.
\end{itemize}
...
Build a fuzzy neighbor network in high-D, embed in low-D using a smooth similarity kernel, and adjust positions to preserve connectivity: connected points stay close, unconnected points stay apart. Unlike t-SNE, UMAP uses a fuzzy graph rather than all pairwise probabilities, making it faster and better at capturing cluster structure and some global topology.
