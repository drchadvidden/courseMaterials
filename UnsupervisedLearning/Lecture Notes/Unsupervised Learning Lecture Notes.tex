...
\item What is AI, ML, data science, statistics, etc? How does it all fit? Cool diagrams. 
...
\item Basic example of linear regression with $p=1$ for student data. Can be used for student intervention (early warning, effect of living on campus, attending class). 
...
\item What types of students are there? What variable combinations belong together (academic, engagement)? Unusual students? Better understand student populations. 
...
\item Dimension reduction: Reduce noise and multicolinearity, data viz, large to smaller data, data understanding. PCA, t-SNE, UMAP, SVD
...
\begin{enumerate}
\item Euclidean distance ($\ell_2$): For points $x, y \in \mathbb{R}^d$,
\[
...
\]
\begin{itemize}
\item Geometry: measures angle between rather than distance, same direction is close, ignores scale.
...
\begin{enumerate}
\item Features on a bigger scale dominate distance calculations. Spend difference vs weekly visit count.
...
\]
\item Results in mean 0, variance 1. Now on a standard normal distribution $N(0,1)$. Preserves order and relative distribution.
...
\begin{itemize}
\item Discretize the data space into a finite grid structure.
\item Emphasize computational efficiency, especially for spatial data.
\end{itemize}
...
\item Domain knowledge plays a central role (feature selection, scaling, similarity choice, interpretation).
...
\item Big picture:
\begin{enumerate}
\item Given data $D = \{x_1,\dots,x_n\} \subset \mathbb{R}^p$, partition into $k$ disjoint clusters $C_1,\dots,C_k$.
\item Each point belongs to exactly one cluster.
\item Each cluster is summarized by a single representative (center).
...
\begin{itemize}
\item This optimization problem is computationally expensive, so a basic algorithm is used.
\item Initialize $c_1,\dots,c_k$ as $k$ random objects from $D$.
\item Cluster assignment step:
\[
x_j \mapsto \arg\min_i \|x_j - c_i \|^2.
\]
\item Centroid update step:
\[
...
\[
\min_{o_1,\dots,o_k} \sum_{j=1}^n \min_{1 \le i \le k} d(x_j, o_i).
\]
\item Distance $d(\cdot,\cdot)$ need not be Euclidean. Note the lack of squared distance.
...
\begin{itemize} 
\item Random initial medoids. Brute force check all updated medoids per assigned cluster.
\item Modification for large data, clustering large applications (CLARA) considers random samples of the dataset.
\end{itemize}
\item Slower than $k$-means due to discrete optimization.
\end{enumerate}
...
\begin{itemize}
\item Objective is non-convex $\Rightarrow$ multiple local minima.
\item Initialization matters (e.g., random vs k-means++).
\item Scaling changes the geometry of $|\cdot|$.
\item Choice of $k$ is a modeling decision, not a statistical estimate.
\end{itemize}

\item When partitioning methods work well
\begin{itemize}
\item Clusters roughly convex and isotropic.
\item Moderate dimension with meaningful distances.
...
\begin{enumerate}
\item Clustering is unsupervised: typically no ground truth labels
\item "Good clustering" is not uniquely defined.
\item Evaluation criteria often encodes the same assumptions as the algorithm.
