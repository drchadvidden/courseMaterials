...
\begin{itemize}
    ...
    \item \textbf{Healthcare:} Find abnormal patient measurements or rare conditions
...
In higher dimensions, we replace simple thresholds with a model of the joint distribution.

\begin{itemize}
    \item \textbf{Gaussian model:} \(x \sim \mathcal{N}(\mu, \Sigma)\)
    
    \item Equivalent distance view (Mahalanobis distance):
    \[
    (x-\mu)^T \Sigma^{-1} (x-\mu) \text{ large} \;\Rightarrow\; \text{anomaly}
    \]
\end{itemize}
