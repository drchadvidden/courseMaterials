...
\item \textbf{Univariate intuition (simple starting point).}

For a single variable, anomalies are values that are unusually large or small relative to the data.

\begin{itemize}
    \item \textbf{IQR rule (boxplot rule):}
    \[
    \text{Anomaly if } x < Q_1 - 1.5\,\text{IQR} \quad \text{or} \quad x > Q_3 + 1.5\,\text{IQR}
    \]
    where \( \text{IQR} = Q_3 - Q_1 \).
    
    \item \textbf{Z-score (normal assumption):}
    \[
    z = \frac{x - \mu}{\sigma}
    \quad \Rightarrow \quad
    |z| \text{ large } \;\Rightarrow\; \text{anomaly}
    \]
    Common rule: flag if \( |z| > 3 \). Can lookup thresholds in a distribution table.
    
    \item Notation differences for a population vs a sample. Probably want sample notation here.
    
    \item Note the mean is skewed by outliers. Median can be used. Don't sweat it here.
\end{itemize}

\item \textbf{Key idea (generalization).}

Assume data are generated from a distribution \(p(x)\).  
Points that are unlikely under this model are anomalies:
\[
p(x) \text{ small} \;\Rightarrow\; \text{anomaly}
\]
\item \textbf{Multivariate extension.}

\textbf{Motivation.} 
In multiple dimensions, checking each variable separately can fail. A point may look typical in every coordinate but still be unusual \emph{as a combination}.  
For example, height and weight may each be typical individually, but an unusual pairing (e.g., very tall and very light) may be rare.  
Thus, we must model the \emph{joint behavior} of the variables.

\medskip

\textbf{From 1D to higher dimensions.}
In one dimension, anomaly detection is based on distance from the mean:
\[
\frac{(x - \mu)^2}{\sigma^2} \text{ large} \;\Rightarrow\; \text{anomaly}
\]
This measures how far \(x\) is from \(\mu\), scaled by variability.

In higher dimensions:
\begin{itemize}
    \item The mean becomes a vector \( \mu \in \mathbb{R}^d \)
    \item Variability becomes a covariance matrix \( \Sigma \in \mathbb{R}^{d \times d} \)
    \item We need a notion of distance that accounts for:
    \begin{itemize}
        \item different scales in each direction
        \item relationships (correlations) between variables
    \end{itemize}
\end{itemize}

\medskip

\textbf{Why Euclidean distance is not enough.}
A naive approach is to use Euclidean distance:
\[
\|x - \mu\|^2
\]
...
\begin{itemize}
    ...
    \item variables are independent (no correlation)
\end{itemize}
In real data, some directions have much larger spread than others, and variables are often correlated.  
Thus, Euclidean distance can misidentify anomalies.

\medskip
\subsection*{Gaussian distributions: 1D and multivariate}

\textbf{1D Gaussian.} For a continuous scalar variable $X$, the normal distribution models how likely values are near the mean $\mu$:
\[
...
\]
\begin{itemize}
    \item $(x-\mu)^2$ measures squared distance from the mean.
    \item Dividing by $2\sigma^2$ scales distance by the variance.
    \item Probability density decreases exponentially as $x$ moves away from $\mu$.
\end{itemize}

\medskip

\textbf{Multivariate Gaussian.} For a continuous vector $X \in \mathbb{R}^d$, we generalize to account for scale and correlations:
\[
...
\]
\begin{itemize}
    \item $\mu \in \mathbb{R}^d$ is the mean vector.
    \item $\Sigma \in \mathbb{R}^{d\times d}$ is the covariance matrix.
    \item $(x-\mu)^T \Sigma^{-1} (x-\mu)$ is the Mahalanobis distance, measuring distance in the geometry of the data.
    \item The exponent generalizes $-(x-\mu)^2/(2\sigma^2)$ from 1D.
    \item Probability decays exponentially with distance from the mean along the principal axes of the data.
\end{itemize}

\medskip

\textbf{Key intuition.} In both cases, probability is highest near the mean and decreases with “distance”:
\begin{itemize}
    \item 1D: distance measured by $(x-\mu)/\sigma$ (z-score)
    \item Multivariate: distance measured by Mahalanobis distance
\end{itemize}
...
(x-\mu)^T \Sigma^{-1}(x-\mu)
\]
is called the \emph{Mahalanobis distance}.

\begin{itemize}
    \item It rescales each direction by its variance
    \item It accounts for correlations between variables
    \item It measures distance relative to the shape of the data cloud
\end{itemize}

\[
\text{Large Mahalanobis distance} \;\Rightarrow\; \text{anomaly}
\]
...
\begin{itemize}
    \item In 1D: intervals around the mean
    \item In 2D: ellipses
    \item In higher dimensions: ellipsoids
\end{itemize}

The covariance matrix \( \Sigma \) determines the shape and orientation of these regions:
\begin{itemize}
...
Normal points lie near the center of these ellipsoids; anomalies lie far outside them.

\medskip

\textbf{Normalization constant (intuition).}
\[
\frac{1}{(2\pi)^{d/2} |\Sigma|^{1/2}}
\]
ensures the total probability integrates to 1.  

\begin{itemize}
    \item \( (2\pi)^{d/2} \) comes from integration in \(d\) dimensions
    \item \( |\Sigma|^{1/2} \) reflects the overall spread (volume) of the data
\end{itemize}

Larger spread $\Rightarrow$ lower peak density.

\medskip

\textbf{Key connections.}
\begin{itemize}
    \item Z-score (1D) $\longleftrightarrow$ Mahalanobis distance (multi-d)
    \item Intervals $\longleftrightarrow$ ellipsoids
    \item Distance-based view $\longleftrightarrow$ probability-based view
\end{itemize}
...
\textbf{Key intuition.}
\[
\text{Probability decays with distance from } \mu \text{, measured in the geometry of the data.}
\]

\item \textbf{More flexible models.}
...
\begin{itemize}
    ...
    \]
    Captures multiple clusters of normal behavior.
\end{itemize}

\item \textbf{Basic procedure.}
\begin{enumerate}
    \item Fit a model \(p(x)\)
    \item Compute a score (e.g., likelihood or distance)
    \item Flag low-probability points
\end{enumerate}

\item \textbf{Thresholding.}
\begin{itemize}
    \item Choose cutoff \( \tau \):
    \[
    p(x) < \tau \;\Rightarrow\; \text{anomaly}
    \]
    \item Often chosen via percentiles (e.g., lowest 1\%)
\end{itemize}

\item \textbf{Takeaway.}
\[
\text{Statistical anomaly detection = “unlikely under the model.”}
\]
