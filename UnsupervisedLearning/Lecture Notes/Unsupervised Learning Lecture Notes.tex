...
\begin{enumerate}
\item PCA is one of the central uses of SVD, providing a data-driven, hierarchical coordinate system for high dimensional correlated data.
\item Assume data $X_{m \times n}$ where each column is a sample.
\item Want an orthogonal basis for the columns of $X$ which capture the most variance. Draw a 2D picture.
\item Preprocess the data by subtracting the mean and setting the variance to unity before performing the SVD. Define the mean matrix
\[
M = \frac{1}{n} \sum_{i=1}^n \vec{x}_i.
\]
Subtracting $M$ from $X$ results in the mean-subtracted data $B$. 
\[
B = X - M
\]
\item The covariance matrix of the columns of $B$ is given by 
\[
S = \frac{1}{n-1} B B^T.
\] 
This is the sample covariance of our data $X$. The sum of diagonals of $S$, denoted $trace(S)$ is the total data variance.
\item For simplicity, assume $X$ is in mean deviation form. We want to find change of variable
\[
X = PY
\]
where $Y$ has uncorrelated rows (features), ordered in terms of importance and where $P$ has orthogonal columns. This can be restated as $P^T X + Y$.
For each column of $X$,
\[
\vec{x}_i = P \vec{y_i}
\]
gives the coordinate vector of $\vec{x}_i$ in basis $P$. 

\item Computing the covariance for $Y$, we have
\[
\frac{1}{n-1} Y Y^T = \frac{1}{n-1} (P^T X)(X^T P) = \frac{1}{n-1} P^T (X X^T) P = P^T S P.
\]
So the needed unitary matrix $P$ is on ethat makes the $Y$ covariance matrix diagonal (and hence uncorrelated). 

\item So, $S = P D P^T$ where $D$ is the diagonal of $\lambda_i$ eigenvalues of $S$ with corresponding eigenvectors as the columns of $P = [ \vec{u}_1 \dots \vec{u}_n ]$. Note that 
\[
trace(S) = trace(P D P^T) = trace(D P P^T) = trace(D)
\]
so eigenvalues give variance per principal component.
\item Full circle, we see PCA is the same as left singular vectors and singular values from SVD.
\[
S P = P D
\]
where again $S = X^T X$.
